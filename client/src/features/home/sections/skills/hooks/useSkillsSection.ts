...
export function useSkillsSection() {
      ...
      if (next.length === 0) setActiveName(null);
        ...
        setActiveName((currentName) =>
          currentName !== skill.name
            ? currentName
            : next[Math.min(prev.indexOf(skill.name), next.length - 1)],
        );
      return next;
    });
  }, []);
  const handleCodeProgressChange = useCallback(
    (progress: CodeTypingProgress) =>
      patchSkillsSectionProgress((current) => ({
        ...current,
        activeMode: "code",
  ...
  const handleTerminalProgressChange = useCallback(
    (progress: TerminalTypingProgress) =>
      patchSkillsSectionProgress((current) => ({
        ...current,
        activeMode: "terminal",
        activeSkillName: progress.skillName,
  ...
  return {
    ...
    resolvedSkill,
    openTabs,
    codeProgress: restoredProgress?.code ?? DEFAULT_SKILLS_SECTION_PROGRESS.code,
    terminalProgress: restoredProgress?.terminal ?? DEFAULT_SKILLS_SECTION_PROGRESS.terminal,
    chipHandlers: Object.fromEntries(mappedSkills.map((s) => [s.name, () => handleChipClick(s)])),
    handleTabClick,
    handleTabClose,
    handleCodeProgressChange,
    handleTerminalProgressChange,
  };
}
