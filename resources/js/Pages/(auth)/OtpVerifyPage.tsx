...
export default function OtpVerifyPage() {
  ...
  return (
    <>
      <SEO title="Admin - OTP Verify" description="Admin OTP verification page." />
      <DialogShell open dialogKey="otp" config={AUTH_CONFIG.otp}>
        <form onSubmit={onSubmit} className={cn('space-y-5')} noValidate>
          ...
          <p className={cn('text-center text-xs')} style={{ color: 'var(--gray-10)' }}>
            ...
            <button type="button" onClick={handleResend} disabled={!cooldown.ready} className={cn('font-semibold transition-colors duration-200 disabled:cursor-not-allowed')} style={{ color: cooldown.ready ? 'var(--blue-11)' : 'var(--gray-9)' }}>
